...
import { Icon, Modal, PrimaryButton, Stack, TextField,Dropdown, IDropdownOption } from "office-ui-fabric-react";
...
> {
  ...
  onChange = (e: any, selection: any) => {
    ...
    this.setState({  Category: selection.key  });
    console.log("State this  Value",this.state.Title,this.state.When,this.state.Where,this.state.Link,this.state.Category);
};
    ...
    return (
      <Stack>
        ...
        <Stack className={styles.main}>
          ...
          <PrimaryButton onClick={() => this.setState({ isEditModal: true })}>Add Link</PrimaryButton>
          <Modal
          ...
          >
            <Stack  style={{ marginLeft: "10px",marginRight:'10px', marginTop: "40px" }} >
              <TextField label='Title of Event' placeholder='Title of Event' value={this.state.Title} onChange={(e: React.ChangeEvent<HTMLInputElement>) => this.setState({ Title: e.target.value })} />
...
/>
              ...
              <TextField label='Where' placeholder='Where' value={this.state.Where} onChange={(e: React.ChangeEvent<HTMLInputElement>) => this.setState({ Where: e.target.value })} />
              <TextField label='Link' placeholder='Link' value={this.state.Link} onChange={(e: React.ChangeEvent<HTMLInputElement>) => this.setState({ Link: e.target.value })} />
              <Dropdown label='Category' placeholder='Category' options={this.dropdownOptions}  />
            </Stack>
            <PrimaryButton style={{ marginLeft: "10px" ,  marginTop: "10px"  }} onClick={() => this.createEvent()}>Add Event</PrimaryButton>
          </Modal>
          <Stack horizontal tokens={{ childrenGap: 40 }}>
            <Stack className={styles.container1}>
            <div className={styles.icon}>
            <Icon iconName="AddEvent" aria-label="Add Online Event Icon" style={{ fontSize: '40px' }} className={styles.iconStyle} />
          </div>
