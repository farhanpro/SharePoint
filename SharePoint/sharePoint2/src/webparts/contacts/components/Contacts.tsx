...
export default class Contacts extends React.Component<IContactsProps, IContactsState> {  
  ...
  componentDidMount(): void {
    sp.web.lists.getByTitle("Contacts").items.select()()
    .then((item:any)=>{
      item.map((item : any)=>{
        this.setState({
          Id:item.Id,
          ...
          ContactsArr:[...this.state.ContactsArr,{"Id":item.Id,"Title":item.Title,"ContactType":item.ContactType,"Contact":item.Contact}]
        })
      })
                    ...
                    <StackItem className={styles.iconStackStyle}>
                      {
                        ...
                        <StackItem className={styles.personStack}>
                                    <Icon className={styles.iconStyle} iconName={Constants.ICONS.PERSON.iconName} />
                                  </StackItem>
                                  :
                        val.ContactType === 'email' ?
                        <StackItem className={styles.emailStack}>
                                      <Icon className={styles.iconStyle} iconName={Constants.ICONS.PHONE.iconName} />
                                    </StackItem>
                                    :
                    ...
                    <Stack style={{ overflow:"hidden",width:'78%'}}>
                      ...
                      <Text className={styles.contactTitle}>{val.Title}</Text>
                        ...
                        <Stack style={{ overflow: "hidden", width: "85%" }}>
                          ...
                          className="tooltipHostStyle"
                          styles={Constants.TOOLTIP_ELLIPSIS_STYLES}>
                            <Link  href={"mailto:" + val.Contact}
                                    className={styles.contactTitle}>
                            </Link>
                            {val.Contact}
                          </TooltipHost>):val.ContactType === "link" ? (
                                <TooltipHost
                                  content={`${val.Contact}`}
                                  className="tooltipHostStyle"
                                  styles={Constants.TOOLTIP_ELLIPSIS_STYLES}
                                ...
                                </TooltipHost>):val.ContactType === "number" ?(
                                  <TooltipHost
                                  content={`${val.Title}`}
                                  className='tooltipHostStyle'
                                  styles={Constants.TOOLTIP_ELLIPSIS_STYLES}
